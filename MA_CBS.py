...
def update_internal_constraints(constraints, list_agent):
    for constraint in constraints[:]:
        for agent in list_agent:
            for agent2 in list_agent:
                if constraint['agent'] == agent and constraint['opponent'] == agent2:
                    constraints.remove(constraint)

    return


def update_external_constraints(constraints, list_agent):
    for constraint in constraints[:]:
        for agent in list_agent:
            if constraint['agent'] == agent:
                    constraint['meta-agent'] = list_agent
...
class MACBSSolver(object):
    ...
    def find_solution(self):
        ...
        while len(self.open_list) != 0:
            ...
            #print("LIST OF META: ", curr['list_meta'])
            #print("current path  :", curr['paths'])

            # FINAL PROJECT
            # Check if merge 2 agents or not by using conflict matrix
            # CASE for 2 meta agents
            # Define a bound B

            # Increment the conflict number of 2 agents
            # conflict_matrix[collision['a1']][collision['a2']] +=1
            agent_1 = collision['a1']
            agent_2 = collision['a2']
            ...
            #print(collision)
            #print(curr['matrix'])
            #print("THIS IS AGENT: ", agent_1, agent_2)
            #print("current collisions: ", curr['collisions'])
            #print("current constraints: ",curr['constraints'])
            # print("constraints when call function: ", constraints)
            ...
            #     ### Merge two agents
            #     ### Update constraint
            #     updated_constraints, update_collisions, meta_agent = merge(curr['constraints'], curr['collisions'], agent_num[0], agent_num[1])
            #     meta_agent_path = []D
            #     # for agent in meta_agent:
                     


            #     ### Call A* search for new_agent
            #     ### current['cost'] = get_sum_of_cost(current['paths'])
            #     ### If current['cost] <= upper_bound:
            ...
            for constraint in constraints:
                ...
                if len(constraint['meta-agent']) > 0:   ##If it is meta agent then run EPEA*
                    ...
                    #print("THIS IS PATH IN EPEA*******: ",meta_paths)
                    if meta_paths != None:
                        index = 0
                        for agent in constraint['meta-agent']:
                            child['paths'][agent] = meta_paths[index] + []
                            index+=1
                        #print("Child pathhhhhhhhhh", child['paths'])
